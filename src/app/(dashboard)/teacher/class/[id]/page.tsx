...
function EnrollmentQueryErrorState({
  message,
}: {
  message: string;
}) {
...
export default async function ClassDetailPage({ params }: Props) {
    ...
    const [classResult, countResult, climateResult, pendingRecsResult, historyCountResult, metrics, auditSignal, redactedVoiceRows] =
        await Promise.all([
            supabase
                .from("classes")
            ...
            supabase
                ...
                .select("id, class_id, content, status, dismissal_reason, action_taken_note, teacher_action_note, communicated_to_students, created_at, updated_at, policy_level, ai_message_draft, actions_json, confidence_score, reasoning, inquiry_mode, fallback_used, priority, alert_sent_at")
                .eq("class_id", id)
                .eq("status", "pending")
            ...
            <div className="space-y-6">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Invite code: {classResult.data.invite_code}
                    </p>
                </div>
